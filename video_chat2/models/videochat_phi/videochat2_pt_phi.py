...
class VideoChat2_pt_phi(Blip2Base):
    """
    VideoChat2 model.
    """
    def __init__(self, config):
        ...
        if debug:
            ...
            self.phi_model = AutoModelForCausalLM.from_config(phi_config, trust_remote_code=True)
        else:
    ...
    def encode_img(self, image):
        ...
        with self.maybe_autocast():
            ...
            inputs_phi = self.phi_proj(query_output.last_hidden_state)
            atts_phi = torch.ones(inputs_phi.size()[:-1], dtype=torch.long).to(image.device)
        return inputs_phi, atts_phi
    ...
    def prompt_wrap(self, img_embeds, atts_img, prompt, use_image=False):
        if prompt:
            ...
            p_before_tokens = self.phi_tokenizer(
                p_before, return_tensors="pt", add_special_tokens=False).to(img_embeds.device)
            p_after_tokens = self.phi_tokenizer(
                p_after, return_tensors="pt", add_special_tokens=False).to(img_embeds.device)
            p_before_embeds = self.phi_model.model.embed_tokens(p_before_tokens.input_ids).expand(batch_size, -1, -1)
            p_after_embeds = self.phi_model.model.embed_tokens(p_after_tokens.input_ids).expand(batch_size, -1, -1)
    ...
    def forward(self, image, text_input):
        ...
        to_regress_tokens = self.phi_tokenizer(
            text,
            return_tensors="pt",
            padding="longest",
            truncation=True,
        ...
        targets = to_regress_tokens.input_ids.masked_fill(
            to_regress_tokens.input_ids == self.phi_tokenizer.pad_token_id, -100
        )
        ...
        bos = torch.ones([batch_size, 1],
                         ...
                         device=to_regress_tokens.input_ids.device) * self.phi_tokenizer.bos_token_id
        bos_embeds = self.phi_model.model.embed_tokens(bos)
        atts_bos = atts_img[:, :1]

        to_regress_embeds = self.phi_model.model.embed_tokens(to_regress_tokens.input_ids)
