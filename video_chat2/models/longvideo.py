...
from cluster.cluster import CTM, TCBlock
...
class VideoChat2_pt(Blip2Base):
    ...
    def initialize_cluster_modules(self, model_args):
        ...
        if self.use_cluster and not hasattr(self, 'ctm0'):
            self.ctm0 = CTM(sample_ratio=64, embed_dim=self.config.mm_hidden_size, dim_out=self.config.mm_hidden_size, k=5)
            self.block0 = TCBlock(dim=self.config.mm_hidden_size, num_heads=8)

            self.ctm1 = CTM(sample_ratio=32, embed_dim=self.config.mm_hidden_size, dim_out=self.config.mm_hidden_size, k=3)
            self.block1 = TCBlock(dim=self.config.mm_hidden_size, num_heads=8)

            self.ctm2 = CTM(sample_ratio=16, embed_dim=self.config.mm_hidden_size, dim_out=self.config.mm_hidden_size, k=3)
            self.block2 = TCBlock(dim=self.config.mm_hidden_size, num_heads=8)

            self.ctm3 = CTM(sample_ratio=1/16, embed_dim=self.config.mm_hidden_size, dim_out=self.config.mm_hidden_size, k=5)
    ...
    def encode_img(self, image):
        ...
        with self.maybe_autocast():
            ...
            token_dict = {'x': image_embeds,
                            'token_num': image_embeds.size(1),
                            'idx_token': torch.arange(image_embeds.size(1))[None, :].repeat(
